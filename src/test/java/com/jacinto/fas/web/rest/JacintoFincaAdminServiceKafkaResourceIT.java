...
import com.jacinto.fas.config.KafkaProperties;
...
class JacintoFincaAdminServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        JacintoFincaAdminServiceKafkaResource kafkaResource = new JacintoFincaAdminServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jacinto-finca-admin-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jacinto-finca-admin-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
